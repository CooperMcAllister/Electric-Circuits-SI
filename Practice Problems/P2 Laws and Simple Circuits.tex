...
\section{KCL with Networks}
Find (a) $I_x$, and (b) $I_y$. The boxes represent circuit networks.\\ \\
\begin{circuitikz}[american]  \draw
(0, 0) -- (0, 2) -- (2, 2) -- (2, 0) -- (0, 0)
(4, 0) -- (4, 2) -- (6, 2) -- (6, 0) -- (4, 0)
(0, 4) -- (0, 6) -- (2, 6) -- (2, 4) -- (0, 4)
(4, 4) -- (4, 6) -- (6, 6) -- (6, 4) -- (4, 4)
(1, 0) to[short, f=$5\textrm{A}$] (1, -1)
(2, 1) to[short] (4, 1)
(5, 0) to[short, f<=$4\textrm{A}$] (5, -1)
(1, 2) to[short, f<=$3\textrm{A}$] (1, 4)
(2, 5) to[short, f=$I_x$] (4, 5)
(4.5, 2) to[short] (4.5, 4) (5.5, 2) to[short] (5.5, 4)
(6, 1) to[short, f=$7\textrm{A}$] (7, 1)
(0, 5 )-- (-1, 5) (-1, 5) to[short, f<=$I_y$] (-2, 4) (-1, 5) to[short, f<=$2\textrm{A}$] (-2, 6)
;
\end{circuitikz} \\ \\
\subsection*{Solution}
Summing the currents entering the network consisting of the bottom-right three boxes:
$3\textrm{A} + I_x - 7\textrm{A} + 4\textrm{A} - 5\textrm{A} = 0 \Rightarrow I_x = 5\textrm{A}$ (a). Summing the currents entering the top-left box:
$2\textrm{A} + I_y - I_x - 3\textrm{A} = 0 \Rightarrow I_y = 6\textrm{A}$ (b).


\section{Finding current with KVL}
Find $I_1$, the power absorbed by the $3.3\textrm{k}\Omega$ resistor, and the power generated by the 10V source. \\ \\
\begin{circuitikz}[american] \draw
...
\begin{circuitikz}[american] \draw
(0, 0) node[ground]{} to[vsource, invert, l=$10\textrm{V}$] (0, 3) to[R, l=$3.3\textrm{k}\Omega$, v=$V_3$] (3, 3) to[short] (6, 3) to[R, l=$2.2\textrm{k}\Omega$, f=$I_1$] (9, 3) to[battery, l=$3\textrm{V}$] (9, 0) to[short] (0, 0)
;
\end{circuitikz} \\ \\
So, $I_1 = \frac{10 \textrm{V} - 3\textrm{V}}{3.3\textrm{k}\Omega + 2.2\textrm{k}\Omega} = 1.27 \ \textrm{mA}$.
\\The voltage on the $3.3\textrm{k}\Omega$ resistor is $V_3 = 1.27\textrm{mA}\cdot3.3\textrm{k}\Omega = 4.191$ V. Therefore the power absorbed is $P = iv = 1.27\textrm{mA}\cdot4.191\textrm{V} = 5.32$ mW. The power generated by the 10V source is $P = -iv = -1.27\textrm{mA}\cdot(-10\textrm{V}) = 12.7$ mW.

\section{KVL Equations}
Find $v_o$ if $r=2\Omega$ and $g=5\textrm{S}$. \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[V=$5\textrm{V}$, invert] (0, 5) to[short] (3, 5) to[open, o-o, v=$v_i$] (3, 3.25) -- (3, 3) to[R=$r$] (3, 0) -- (0, 0)
(3, 0) -- (9, 0) to[cI, l=$g\cdot v_i$] (9, 3) to[R] (6, 3) to[R=$10\Omega$] (3, 3)
(6, 0) to[open, v<=$v_o$] (6, 3)
;
\end{circuitikz} \\ \\
\subsection*{Solution}
We can find the voltage across the resistor in two ways: using KVL, and using Ohm's Law. Let's first label this voltage: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[V=$5\textrm{V}$, invert] (0, 5) to[short] (3, 5) to[open, o-o, v=$v_i$] (3, 3.25) -- (3, 3) to[R=$r$, v=$V_r$] (3, 0) -- (0, 0)
(3, 0) -- (9, 0) to[cI, l=$g\cdot v_i$] (9, 3) to[R] (6, 3) to[R=$10\Omega$] (3, 3)
(6, 0) to[open, v<=$v_o$] (6, 3)
;
\end{circuitikz} \\ \\
Using KVL, $V_r = -v_i + 5\textrm{V}$. Using Ohm's Law, $V_r = g\cdot v_i \cdot r$. Equating the two, we have:
$-v_i + 5\textrm{V} = 5\textrm{S}\cdot v_i \cdot 2\Omega$. Solving, $v_i = \frac{5}{11}\textrm{V}$. This means that the current $g\cdot v_i = \frac{25}{11}$ A. Next, we can use KVL to find: $v_o = g\cdot v_i\cdot 10\Omega + V_r = \frac{25}{11}\cdot 10 + \frac{25}{11}\cdot 2 = 27.3\textrm{V}$.
...
Find $R_{eq}$. \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[short] (0, 3) to[open, o-o] (0, 6) (3, 0) to[R, l=$30\Omega$] (3, 3) to[R, l=$5\Omega$] (3, 6) (0, 3) to[R, l=$30\Omega$] (3, 3)
(6, 0) to[R, l=$50\Omega$] (6, 3) to[R, l=$60\Omega$] (6, 6) to[R, l=$60\Omega$] (9, 6) to[short] (9, 3) to[R, l=$200\Omega$] (9, 0) (6, 3) to[short] (9, 3) (6, 6) to[R, l=$60\Omega$] (9, 3)
(0, 0) to[short] (9, 0) (0, 6) to[short] (6, 6)
;
\draw (-1, 4.5) -- (-0.5, 4.5) [->];
\node[draw=none] at (-1.5, 4.5) {$R_{eq}$};
\end{circuitikz} \\ \\
\subsection*{Solution}
We can combine parallel resistors to obtain: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[short] (0, 3) to[open, o-o] (0, 6) (3, 0) to[R, l=$15\Omega$] (3, 3) to[R, l=$5\Omega$] (3, 6)
(6, 0) to[R, l=$40\Omega$] (6, 3) to[R, l=$20\Omega$] (6, 6) 
(0, 0) to[short] (6, 0) (0, 6) to[short] (6, 6)
;
\draw (-1, 4.5) -- (-0.5, 4.5) [->];
\node[draw=none] at (-1.5, 4.5) {$R_{eq}$};
\end{circuitikz} \\ \\
Then, combining series resistors, \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[open, o-o] (0, 3) (3, 0) to[R, l=$20\Omega$] (3, 3)
(6, 0) to[R, l=$60\Omega$] (6, 3) 
(0, 0) to[short] (6, 0) (0, 3) to[short] (6, 3)
;
\draw (-1, 1.5) -- (-0.5, 1.5) [->];
\node[draw=none] at (-1.5, 1.5) {$R_{eq}$};
\end{circuitikz} \\ \\
Finally, combining parallel resistors, $R_{eq} = 15\Omega$.
...
Find $V_x$. \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[short] (0, 3) to[isource, l=$10\textrm{mA}$] (0, 6) (3, 0) to[R, l=$40\Omega$, v<=$V_1$]  (3, 3) to[R, l=$5\Omega$] (3, 6) (0, 3) to[R, l=$40\Omega$] (3, 3)
(6, 0) to[R, l=$50\Omega$, v<=$V_2$] (6, 3) to[R, l=$75\Omega$] (6, 6) to[R, l=$75\Omega$] (9, 6) to[short] (9, 3) to[R, l=$200\Omega$] (9, 0) (6, 3) to[short] (9, 3) (6, 6) to[R, l=$75\Omega$] (9, 3)
(3, 3) to[open, v=$V_x$] (6, 3)
(0, 0) to[short] (9, 0) (0, 6) to[short] (6, 6)
(9, 0) -- (12, 0) to[R=$260\Omega$] (12, 7) -- (6, 7) -- (6, 6)
;
\end{circuitikz} \\ \\
\subsection*{Solution}
We will solve this problem by finding first $V_1$ and $V_2$.
We can combine resistors to obtain: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[isource, l=$10\textrm{mA}$] (0, 3) -- (3, 3) to[R=$25\Omega$, f=$I_1$] (3, 0) -- (0, 0) (3, 3) -- (6, 3) to[R=$52\Omega$, f=$I_2$, v=$V_{tot}$] (6, 0) -- (3, 0)
;
\end{circuitikz} \\ \\
Using current division and KCL,
$$I_1 = 10\textrm{mA}\cdot\frac{\frac{1}{25}}{\frac{1}{25} + \frac{1}{52}} = 6.7532\textrm{mA}$$
$$I_2 = 10\textrm{mA} - I_1 = 3.2467\textrm{mA}$$
We can also find the total voltage $V_{tot} = I_2\cdot 52\Omega = 168.831\textrm{mV}$.
\\We can now use Ohm's Law to find $V_1$, after combining the two $20\Omega$ resistors: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[R=$20\Omega$, v<=$V_1$] (0, 3) to[R=$5\Omega$, f<=$I_1$] (0, 6)
;
\end{circuitikz} \\ \\
$V_1 = I_1\cdot 20\Omega = 135.064\textrm{mV}$.
Next, we can use voltage division to find $V_3$: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) -- (1, 0) to[R=$40\Omega$, v<=$V_2$] (1, 3) to[R=$25\Omega$,] (1, 6) -- (0, 6) to[open, v=$V_{tot}$] (0, 0)
;
\end{circuitikz} \\ \\
$V_2 = V_{tot}\cdot\frac{40}{40 + 25} = 103.896\textrm{mV}$.
Finally, using KVL, $V_x = V_1 - V_2 = 31.2\textrm{mV}$.
