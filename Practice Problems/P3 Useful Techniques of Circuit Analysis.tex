...
;
\end{circuitikz} \\ \\
The current through the $10\Omega$ resistor can be found using current division and KCL: $I_x = (6 \textrm{A} + 7 \textrm{A} - 3\textrm{A}) \cdot\frac{\frac{1}{10}}{\frac{1}{10} + \frac{1}{15} + \frac{1}{5}} = 2.727 \textrm{A}$. Now the voltage can be found with Ohm's Law: $V_x = 2.727 \textrm{A} \cdot 10\Omega = 27.27 \ \textrm{V}$. 
...
\end{circuitikz} \\ \\
\subsection*{Solution}
To make the analysis easier, we can first simplify parallel  resistors: \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[short] (0, 3) to[isource, l=$10\textrm{mA}$] (0, 6) (3, 0) to[R, l=$20\Omega$, v<=$V_1$] (3, 3) to[R, l=$5\Omega$] (3, 6) 
(6, 0) to[R, l=$40\Omega$, v<=$V_2$] (6, 3) to[R, l=$25\Omega$] (6, 6) 
(3, 3) to[open, v=$V_x$] (6, 3)
(0, 0) to[short] (6, 0) (0, 6) to[short] (6, 6)
(6,0) -- (9, 0) to[R=$260\Omega$, v<=$V_3$] (9, 6) -- (6, 6)
;
\end{circuitikz} \\ \\
Setting the bottom node as the reference, we can now write the following KCL equations for nodes of voltage $V_1$, $V_2$, and $V_3$:
$$\frac{V_3 - V_1}{5\Omega} + \frac{0 - V_1}{20\Omega} = 0$$
$$\frac{V_3 - V_2}{25\Omega} + \frac{0 - V_2}{40\Omega} = 0$$
$$10\textrm{mA} + \frac{V_1-V_3}{5\Omega} + \frac{V_2-V_3}{25\Omega} + \frac{0 - V_3}{260\Omega} = 0$$
Solving, $V1 = 135.065\textrm{mV}$ and $V_2 = 103.896\textrm{mV}$. Therefore, $V_x = V_1 - V_2 = 31.2\textrm{mV}$.
