...
Find $V_x$, $V_y$, $V_z$, $V_a$, $I_x$, and $I_y$. \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[vsource, l=$8\textrm{V}$] (0, 3) to[short] (0, 5) to[isource, l=$20\textrm{mA}$] (3, 5) to[short] (12, 5) to[short, f=$I_y$] (12, 3) to[short] (12, 0) to[vsource, l=$5 \textrm{V}$] (9, 0) to[short] (0, 0)
(0, 3) to[short] (3, 3) to[R, v=$V_a$] (6, 3) to[isource, invert, l=$50\textrm{mA}$] (9, 3) to[short] (12, 3)
(3, 0) to[R, l=$20\Omega$, v=$V_x$, f=$I_x$] (3, 3) 
(6, 0) to[short] (6, 3)
(9, 0) to[R] (9, 3)
...
$V_a =- 8 \ \textrm{V}$ \\
$I_y = 20 \ \textrm{mA}$ \\
Using Ohm's Law, $I_x = \frac{V_x}{20\Omega} = \frac{8\textrm{V}}{20\Omega} = 400 \textrm{mA}$.
...
If $V_y = 0.85\textrm{V}$ and $g = 1 \textrm{mA}/\textrm{V}$, find (a) $I_x$ and (b) $V_{BA}$. \\ \\
\begin{circuitikz}[american] \draw
(0, 0) to[generic, f<=$I_x$] (0, 6) to[cI, l=$g\cdot V_x$] (6, 6) to[short, *-*] (6, 0) to[short, *-*] (0, 0)
(3, 0) to[cV, invert, l=$V_x$] (3, 3) to[generic, v=$V_y$] (6, 3);
\draw (0, 0) node[left] {A}; \draw (6, 6) node[right] {B}; \draw (6, 0) node[right] {C}; 
\end{circuitikz}
\subsection*{Solution}
Note that $V_x = V_y$, and $I_x = -g\cdot V_x$. Therefore, $I_x = -g\cdot V_y = -1\textrm{mA}/\textrm{V}\cdot 0.85\textrm{V} = -850 \mu\textrm{A}$ (a). Because A, B, and C are all  on the same node, $V_{BA} = 0 \textrm{V}$ (b).
